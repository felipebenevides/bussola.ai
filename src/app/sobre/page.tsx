...
// Para trocar a foto: jogue um arquivo em `public/felipe.jpg` (ou .png) com
// no mínimo 600x600 quadrado. O componente Avatar tenta carregar essa imagem
// e cai pro placeholder com iniciais se ela não existir.
...
const PROJECTS = [
  ...
  },
  {
    ...
    stack: ["C#", "Elixir", "Python", "Node.js", "RabbitMQ", "PostgreSQL", "Redis", "AWS", "DataDog", "Next.js", "Flutter"],
    ...
    <main className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <section className="relative overflow-hidden border-b border-zinc-900">
        ...
        <div className="relative mx-auto grid w-full max-w-6xl gap-10 px-6 py-16 sm:py-24 md:grid-cols-[auto_1fr] md:items-center md:gap-14">
          ...
          <div className="space-y-6">
            <div className="flex flex-wrap items-center gap-2 text-xs font-medium uppercase tracking-[0.18em] text-emerald-400">
            ...
            <p className="max-w-2xl text-base leading-relaxed text-zinc-300 sm:text-lg">
              Engenheiro de software com trajetória do <strong className="text-white">back/full-stack</strong>{" "}
              até liderança — Tech Lead e Engineering Manager. Setores de alta criticidade:{" "}
              meios de pagamento, mercado financeiro, imobiliário, fidelização e seguros.{" "}
              Paixão por <strong className="text-white">IA aplicada</strong> e arquiteturas
              complexas, sem dogma — monolito quando o contexto pede.
            </p>

            <div className="flex flex-wrap gap-3 pt-2">
              <a
                href="mailto:felipebenevides@outlook.com"
                className="inline-flex h-11 items-center gap-2 rounded-lg bg-emerald-500 px-5 text-sm font-semibold text-zinc-950 shadow-lg shadow-emerald-500/20 transition-all hover:-translate-y-0.5 hover:bg-emerald-400"
              >
              ...
              <a
                ...
                className="inline-flex h-11 items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900/60 px-5 text-sm font-medium text-zinc-100 transition-all hover:-translate-y-0.5 hover:border-zinc-700 hover:bg-zinc-900"
              >
              ...
              <Link
                href="/tutor"
                className="inline-flex h-11 items-center gap-2 rounded-lg border border-emerald-900/60 bg-emerald-950/40 px-5 text-sm font-medium text-emerald-200 transition-all hover:-translate-y-0.5 hover:border-emerald-700 hover:bg-emerald-950/70"
              >
            ...
            <ul className="flex flex-wrap gap-x-6 gap-y-2 pt-4 text-sm text-zinc-400">
          ...
          <div>
            ...
            <ul className="mt-6 space-y-3">
              {CERTIFICATIONS.map((c) => (
                <li
                  key={c.title}
                  className="flex items-start gap-3 rounded-xl border border-zinc-900 bg-zinc-900/40 p-4"
                >
                  <span className="mt-0.5 flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-amber-500/20 to-orange-600/10 text-amber-400">
            ...
            <ul className="mt-6 space-y-3">
              {EDUCATION.map((e) => (
                <li
                  ...
                  className="flex items-start gap-3 rounded-xl border border-zinc-900 bg-zinc-900/40 p-4"
                >
                  <span className="mt-0.5 flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-sky-500/20 to-indigo-600/10 text-sky-400">
      ...
      <footer className="bg-zinc-950">
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 py-12 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="text-sm font-semibold text-zinc-100">Felipe Benevides</div>
        ...
        <div className="border-t border-zinc-900 py-4 text-center text-[10px] text-zinc-600">
...
function BackgroundGlow() {
  return (
    <>
      <div className="pointer-events-none absolute -left-32 -top-32 h-80 w-80 rounded-full bg-emerald-500/20 blur-3xl" />
...
function StackCard({
  ...
  category: (typeof STACK)[number];
}) {
  const Icon = category.icon;
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-zinc-900 bg-zinc-900/40 p-5 transition-all hover:-translate-y-0.5 hover:border-zinc-800 hover:bg-zinc-900/70">
      <div
      ...
      <div className="relative">
        ...
        <h3 className="mt-4 text-sm font-semibold text-zinc-100">{category.title}</h3>
        <div className="mt-3 flex flex-wrap gap-1.5">
          {category.items.map((item) => (
            <span
              key={item}
              className="rounded-md border border-zinc-800 bg-zinc-950/60 px-2 py-0.5 text-[11px] text-zinc-300"
            >
              {item}
            </span>
          ))}
...
function ProjectRow({
  ...
  return (
    <article className="group relative overflow-hidden rounded-2xl border border-zinc-900 bg-gradient-to-br from-zinc-900/60 to-zinc-950/40 p-6 transition-all hover:-translate-y-0.5 hover:border-emerald-900/60 hover:from-zinc-900/80">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 flex-1 space-y-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <h3 className="text-xl font-bold tracking-tight text-zinc-50">{project.name}</h3>
            <span className="rounded-full bg-zinc-900 px-2.5 py-0.5 text-[11px] text-zinc-400">
              {project.period}
            </span>
          </div>
          <p className="text-sm font-medium text-emerald-300">{project.role}</p>
          <p className="text-sm leading-relaxed text-zinc-300">{project.description}</p>
          <div className="flex flex-wrap gap-1.5 pt-1">
            {project.stack.map((s) => (
              <span
                key={s}
                className="rounded-md border border-zinc-800 bg-zinc-950/80 px-2 py-0.5 text-[10px] font-medium text-zinc-400"
              >
                {s}
              </span>
            ))}
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {project.clients.map((c) => (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 rounded-md border border-emerald-900/50 bg-zinc-950/60 px-2 py-1 text-[11px] text-emerald-200 transition-colors hover:border-emerald-700 hover:bg-emerald-950/60"
                  >
                    <span>{c.name}</span>
          ...
          <div className="shrink-0">
            {project.external ? (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex h-10 items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900 px-4 text-xs font-semibold text-zinc-100 transition-colors hover:border-emerald-800 hover:text-emerald-300"
              >
                {project.cta}
              ...
              <Link
                href={project.href}
                className="inline-flex h-10 items-center gap-2 rounded-lg bg-emerald-500 px-4 text-xs font-semibold text-zinc-950 transition-colors hover:bg-emerald-400"
              >
                {project.cta}
                <ArrowUpRight className="h-3.5 w-3.5" />
              </Link>
...
function FooterLink({
  ...
  const className =
    "inline-flex items-center gap-1.5 rounded-md border border-zinc-900 bg-zinc-900/40 px-2.5 py-1.5 text-zinc-300 transition-colors hover:border-zinc-800 hover:bg-zinc-900 hover:text-emerald-300";
  const content = (
    <>
