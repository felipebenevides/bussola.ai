...
const TurnSchema = z.object({
  ...
  complete: z
    .boolean()
    .describe(
      "true APENAS quando você já coletou os 4 dados obrigatórios (goal, time_per_day, deadline, weak_area) e está mandando a mensagem final de confirmação."
    ),
  profile: z
    .object({
...
const SYSTEM_PROMPT = `Você é a Bússola, tutora de IA da CEFIS. Seu papel agora é fazer um onboarding curto e acolhedor (máximo 4 perguntas) para entender o aluno.
...
- Quando já tiver os 4 dados: complete=true + next_message de confirmação ("Show, anotei tudo. Vou montar seu plano agora →") + profile preenchido.
